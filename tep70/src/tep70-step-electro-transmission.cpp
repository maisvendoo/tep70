...
void TEP70::stepElectroTransmission(const double& t, const double& dt)
{
    ...
    bool is_819_ON1 = cabine_switcher->isCabine1() &&
                      km[CAB1]->isNoZero() &&
                      brake_lock[CAB1]->isStateOn() &&
                      epk[CAB1]->isKeyOn() && azv_upr_tepl[CAB1].getState();

    bool is_819_ON2 = cabine_switcher->isCabine2() &&
                      km[CAB2]->isNoZero() &&
                      brake_lock[CAB2]->isStateOn() &&
                      epk[CAB2]->isKeyOn() && azv_upr_tepl[CAB2].getState();
    ...
    bool is_Revers_Forward = is_819_ON && (km[CAB1]->isForward() || km[CAB2]->isBackward());

    // Цепь вентиля реверсора назад
    bool is_Revers_Backward = is_819_ON && (km[CAB1]->isBackward() || km[CAB2]->isForward());
